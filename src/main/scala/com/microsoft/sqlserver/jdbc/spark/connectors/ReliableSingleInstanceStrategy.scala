...
import com.microsoft.sqlserver.jdbc.spark.failureinjection.FailureInjection
import com.microsoft.sqlserver.jdbc.spark.utils.BulkCopyUtils
import com.microsoft.sqlserver.jdbc.spark.utils.BulkCopyUtils.{executeUpdate, savePartition}
...
/**
 * Implements the Reliable write strategy for Single Instances that's that's
 * resilient to executor restart.
 * Write is implemented as a 2 phase commit and executor restarts do not
 * result in duplicate inserts.
 */
object ReliableSingleInstanceStrategy
  ...
  def write(
    ...
    phase1Success match {
      ...
      case false =>
        ...
        cleanupStagingTables(conn, stagingTableList, options)
        throw new SQLException(s"""Failed dues to non-transient error. No records written """)
    }
    ...
    try {
      df.rdd
        .mapPartitionsWithIndex((index, iterator) => {
          ...
          val newOptions =
            new SQLServerBulkJdbcOptions(options.parameters + ("tableLock" -> "true"))
          idempotentInsertToTable(iterator, table_name, dfColMetadata, newOptions)
          logInfo(
            s"writeToStagingTables: Successfully wrote partition $index to Table $table_name")
          Iterator[Int](1)
        })
        .collect()
    } catch {
   ...
   */
  private def getStagingTableNames(appId: String, nrOfPartitions: Int): IndexedSeq[String] = {
  ...
  private def stmtInsertWithUnion(
    ...
    logDebug(
      s"stmtInsertWithUnion: Staging tables to union are ${stagingTableList.mkString(",")}")
  ...
  private def cleanupStagingTables(
    ...
    stagingTableList.map(item =>
      try {
        JdbcUtils.dropTable(conn, item, options)
      } catch {
        case ex: SQLException =>
          logError(
            s"cleanupStagingTables: Exception while dropping table $item :"
              + ex.getMessage)
    })
  }
  ...
  private def createStagingTable(
      ...
      options: SQLServerBulkJdbcOptions): Unit = {
    logDebug(
      s"createStagingTable : Creating table $tableName as schema copy of ${options.dbtable}")
  ...
  private def createStagingTables(
    ...
    stagingTableList.map(item =>
      try {
        ...
        case ex: SQLException =>
          logError(
            s"createStagingTables: Exception while creating table $item : " + ex.getMessage())
          throw ex
    })
  }
  def strategy(): Int = {
    SQLServerBulkJdbcOptions.NO_DUPLICATES
  }
