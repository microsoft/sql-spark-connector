...
class DataSourceTest extends FunSuite with Matchers {
    ...
    test("JdbcBulkOptions should have proper Bulk configurations") {
        ...
        assert(options.allowEncryptedValueModifications == params("allowEncryptedValueModificationS").toBoolean)
        ...
        assert(options.driverClass == "com.microsoft.sqlserver.jdbc.SQLServerDriver")
    }

    test("Data pool URL generation") {
        val urlParams = "database=spark_mssql_db;user=testusera1;password=mypass;encrypt=false;trustServerCertificate=true;"
    ...
    test("Data pool options test") {
        ...
        var options = new SQLServerBulkJdbcOptions(Map(
               "urL" -> "jdbc:sqlserver://myUrl", "dbtablE"-> "myTable"))
        assert(false == DataPoolUtils.isDataPoolScenario(options))

        options = new SQLServerBulkJdbcOptions(Map(
            "urL" -> "jdbc:sqlserver://myUrl", "dbtablE"-> "myTable",
            "dataPoolDataSource"->""))
        assert(false == DataPoolUtils.isDataPoolScenario(options))
