...
class MasterInstanceTest(testUtils:Connector_TestUtils) {
    ...
    def test_gci_read_write() {
        ...
        val result_schema = testUtils.emptyDfColsSchemaMetadata(result.schema)
...
//    }
    ...
    */
    ...
    def test_gci_write_parallel() {
        ...
        val result1_schema = testUtils.emptyDfColsSchemaMetadata(result1.schema)
        assert(df.schema == result1_schema)
        var result2 = testUtils.df_read(table_name2)
        val result2_schema = testUtils.emptyDfColsSchemaMetadata(result2.schema)
        assert(df.schema == result2_schema)
